...
class WarcWriter:
        ...
        if dedup_info is not None:
            ...
            if recorded_url.response_recorder.payload_offset is not None:
                response_header_block = recorded_url.response_recorder.tempfile.read(
                    recorded_url.response_recorder.payload_offset)
            else:
                response_header_block = recorded_url.response_recorder.tempfile.read()
    ...
    def digest_str(self, hash_obj):
        ...
        base64.b32encode(hash_obj.digest()) if self.base32 else hash_obj.hexdigest().encode('ascii'))
    ...
    def _writer(self):
        ...
        if self._fpath and self.rollover_time and datetime.utcnow() - self._current_rollover_time > timedelta(
                seconds=self.rollover_time):
        ...
        if self._fpath and os.path.getsize(self._fpath) > self.rollover_size:
            self.logger.debug("Closing WARC because exceeded size.")
            self.close_writer()
...
class WarcWriterThread(threading.Thread):
    ...
    def run(self):
        self.logger.info(
            'WarcWriterThread starting, directory={} gzip={} rollover_size={} rollover_idle_time={} rollover_time={} prefix={} port={}'.format(
                os.path.abspath(self.warc_writer.directory), self.warc_writer.gzip, self.warc_writer.rollover_size,
                self.warc_writer.rollover_time, self.rollover_idle_time, self.warc_writer.prefix, self.warc_writer.port))

        self._last_sync = self._last_activity = time.time()

        queue_empty = True
        while not self.stop.is_set() or not queue_empty:
            try:
                queue_empty = False
                recorded_url = self.recorded_url_q.get(block=True, timeout=0.5)
                self.logger.info(
                    "recorded_url.warcprox_meta={} for {}".format(recorded_url.warcprox_meta, recorded_url.url))
                ...
                if (self.warc_writer._fpath is not None
                    ...
                    self.logger.debug(
                        'rolling over warc file after {} seconds idle'.format(time.time() - self._last_activity))
                    self.warc_writer.close_writer()

                # Rollover based on time
                if self.warc_writer._fpath and self.warc_writer.rollover_time and datetime.utcnow() - self.warc_writer._current_rollover_time > timedelta(seconds=self.warc_writer.rollover_time):
                    self.logger.debug("Closing WARC because exceeded rollover time.")
                    self.warc_writer.close_writer()
